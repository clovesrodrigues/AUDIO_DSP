...
O projeto tenta habilitar `CV_GUI` automaticamente em Windows com `-DTAPE_SATURATION_VST3_ENABLE_CV_GUI=ON`.
...
```bash
cmake -S examples/tape_saturation_vst3 -B build/tape_saturation_vst3 -G "MinGW Makefiles"
cmake --build build/tape_saturation_vst3 --parallel
```

Em Windows, o artefato é emitido em:

```text
build/tape_saturation_vst3/out/tape_saturation_vst3/tape_saturation_vst3.vst3
```
