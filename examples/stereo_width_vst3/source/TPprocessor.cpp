...
StereoWidthVST3Processor::StereoWidthVST3Processor ()
{
    setControllerClass (kStereoWidthVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API StereoWidthVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API StereoWidthVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API StereoWidthVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API StereoWidthVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API StereoWidthVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API StereoWidthVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
    ...
    (void)parameters_.setImmediateReal (kParamStereoWidthVST3MidGain, midGain);
    (void)parameters_.setImmediateReal (kParamStereoWidthVST3SideGain, sideGain);
    (void)parameters_.setImmediateReal (kParamStereoWidthVST3Width, width);
    applyParametersToDSP ();
    return kResultOk;
}

tresult PLUGIN_API StereoWidthVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void StereoWidthVST3Processor::registerParameters () noexcept
{
    using namespace cvdsp::manager;
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamStereoWidthVST3MidGain, "Mid Gain", "Mid Gain",
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamStereoWidthVST3SideGain, "Side Gain", "Side Gain",
        ParameterUnit::None, ParameterScale::Linear, kParamFlags,
        {0.0f, 2.0f, 1.0f, 0.0f, 1.0f}, nullptr, 0, "side_gain", "", "Spatial", 2),
        ParameterSmoothingMode::Linear);
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamStereoWidthVST3Width, "Stereo Width", "Stereo Width",
        ParameterUnit::None, ParameterScale::Linear, kParamFlags,
        {0.0f, 2.0f, 1.0f, 0.0f, 1.0f}, nullptr, 0, "stereo_width", "", "Spatial", 2),
        ParameterSmoothingMode::Linear);
}

void StereoWidthVST3Processor::applyParametersToDSP () noexcept
{
    midGain_ = parameters_.getCurrentReal (kParamStereoWidthVST3MidGain);
    sideGain_ = parameters_.getCurrentReal (kParamStereoWidthVST3SideGain);
    width_ = parameters_.getCurrentReal (kParamStereoWidthVST3Width);
}

void StereoWidthVST3Processor::processStereoBlock (const Vst::Sample32* leftIn,
                                                ...
                                                int32 sampleCount) noexcept
{
    ...
    {
        const auto ms = cvdsp::spatial::MidSide<float>::encode (leftIn[sample], rightIn[sample]);
        const auto lrPre = cvdsp::spatial::MidSide<float>::decode (ms.mid * midGain_, ms.side * sideGain_);
        const auto lr = processor_.process (lrPre.left, lrPre.right, width_);
