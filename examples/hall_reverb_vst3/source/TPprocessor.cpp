...
HallReverbVST3Processor::HallReverbVST3Processor ()
{
    setControllerClass (kHallReverbVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API HallReverbVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API HallReverbVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API HallReverbVST3Processor::setActive (TBool state)
{
    if (state)
        resetDSP ();
...
tresult PLUGIN_API HallReverbVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API HallReverbVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API HallReverbVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API HallReverbVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
tresult PLUGIN_API HallReverbVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void HallReverbVST3Processor::registerParameters () noexcept
{
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        ...
        ParameterRange<float> {0.0f, 1.0f, 0.25f}, nullptr, 0, "mix", "%", "Hall Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamDecay, "Decay", "Decay", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.50f}, nullptr, 0, "decay", "%", "Hall Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamDamping, "Damping", "High-Frequency Damping", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.35f}, nullptr, 0, "damping", "%", "Hall Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamPreDelay, "PreDelay", "Pre-Delay", ParameterUnit::Milliseconds, ParameterScale::Linear, kParamFlags,
        ParameterRange<float> {0.0f, 250.0f, 20.0f}, nullptr, 0, "predelay", "ms", "Hall Reverb", 1));
}

void HallReverbVST3Processor::prepareDSP () noexcept
{
    dsp_.prepare (dspContext_);
}

void HallReverbVST3Processor::resetDSP () noexcept
{
    dsp_.reset ();
}

void HallReverbVST3Processor::applyParametersToDSP () noexcept
{
    ...
    dsp_.setRT60 (denormalize (decay_, 0.30f, 12.0f));
    dsp_.setDamping (damping_);
    dsp_.setPreDelay (preDelayMs_);
    dsp_.setHallSize (decay_);
    dsp_.setWidth (0.85f);
}
