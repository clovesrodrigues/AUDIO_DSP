...
RoomReverbVST3Processor::RoomReverbVST3Processor ()
{
    setControllerClass (kRoomReverbVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API RoomReverbVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API RoomReverbVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API RoomReverbVST3Processor::setActive (TBool state)
{
    if (state)
        resetDSP ();
...
tresult PLUGIN_API RoomReverbVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API RoomReverbVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API RoomReverbVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API RoomReverbVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
tresult PLUGIN_API RoomReverbVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void RoomReverbVST3Processor::registerParameters () noexcept
{
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        ...
        ParameterRange<float> {0.0f, 1.0f, 0.25f}, nullptr, 0, "mix", "%", "Room Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamDecay, "Decay", "Decay", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.50f}, nullptr, 0, "decay", "%", "Room Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamDamping, "Damping", "High-Frequency Damping", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.35f}, nullptr, 0, "damping", "%", "Room Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamPreDelay, "PreDelay", "Pre-Delay", ParameterUnit::Milliseconds, ParameterScale::Linear, kParamFlags,
        ParameterRange<float> {0.0f, 250.0f, 20.0f}, nullptr, 0, "predelay", "ms", "Room Reverb", 1));
}

void RoomReverbVST3Processor::prepareDSP () noexcept
{
    dsp_.prepare (dspContext_);
}

void RoomReverbVST3Processor::resetDSP () noexcept
{
    dsp_.reset ();
}

void RoomReverbVST3Processor::applyParametersToDSP () noexcept
{
    ...
    dsp_.setDecayTime (denormalize (decay_, 0.05f, 1.20f));
