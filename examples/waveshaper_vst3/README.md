...
Exemplo VST3 usando `CV_DSP/Saturation/Waveshaper.hpp` e a classe `cvdsp::saturation::Waveshaper<float>`.
...
O projeto tenta habilitar `CV_GUI` automaticamente em Windows com `-DWAVESHAPER_VST3_ENABLE_CV_GUI=ON`.
...
```bash
cmake -S examples/waveshaper_vst3 -B build/waveshaper_vst3 -G "MinGW Makefiles"
cmake --build build/waveshaper_vst3 --parallel
```

Em Windows, o artefato é emitido em:

```text
build/waveshaper_vst3/out/waveshaper_vst3/waveshaper_vst3.vst3
```
