...
O projeto tenta habilitar `CV_GUI` automaticamente em Windows com `-DTUBE_SATURATION_VST3_ENABLE_CV_GUI=ON`.
...
```bash
cmake -S examples/tube_saturation_vst3 -B build/tube_saturation_vst3 -G "MinGW Makefiles"
cmake --build build/tube_saturation_vst3 --parallel
```

Em Windows, o artefato é emitido em:

```text
build/tube_saturation_vst3/out/tube_saturation_vst3/tube_saturation_vst3.vst3
```
