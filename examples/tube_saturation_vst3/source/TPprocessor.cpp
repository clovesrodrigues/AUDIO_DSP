...
TubeSaturationVST3Processor::TubeSaturationVST3Processor ()
{
    setControllerClass (kTubeSaturationVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API TubeSaturationVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API TubeSaturationVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API TubeSaturationVST3Processor::setActive (TBool state)
{
    if (state)
    {
...
tresult PLUGIN_API TubeSaturationVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API TubeSaturationVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API TubeSaturationVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API TubeSaturationVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
    ...
    (void)parameters_.setImmediateReal (kParamTubeSaturationDrive, drive);
    (void)parameters_.setImmediateReal (kParamTubeSaturationBias, bias);
    (void)parameters_.setImmediateReal (kParamTubeSaturationMix, mix);
    applyParametersToDSP ();
    return kResultOk;
}

tresult PLUGIN_API TubeSaturationVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void TubeSaturationVST3Processor::registerParameters () noexcept
{
    using namespace cvdsp::manager;
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamTubeSaturationDrive, "Drive", "Drive",
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamTubeSaturationBias, "Bias", "Bias",
        ParameterUnit::None, ParameterScale::Linear, kParamFlags,
        {-1.0f, 1.0f, 0.0f, 0.0f, 1.0f}, nullptr, 0, "bias", "", "Saturation", 2),
        ParameterSmoothingMode::Linear);
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamTubeSaturationMix, "Mix", "Mix",
        ParameterUnit::Percent, ParameterScale::Linear, kParamFlags,
        {0.0f, 1.0f, 1.0f, 0.0f, 1.0f}, nullptr, 0, "mix", "", "Saturation", 2),
        ParameterSmoothingMode::Linear);
}

void TubeSaturationVST3Processor::applyParametersToDSP () noexcept
{
    drive_ = parameters_.getCurrentReal (kParamTubeSaturationDrive);
    bias_ = parameters_.getCurrentReal (kParamTubeSaturationBias);
    mix_ = parameters_.getCurrentReal (kParamTubeSaturationMix);
...
float TubeSaturationVST3Processor::processSample (float input, std::size_t channel) noexcept
{
