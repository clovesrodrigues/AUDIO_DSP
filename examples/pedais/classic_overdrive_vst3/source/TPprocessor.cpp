...
ClassicOverdriveVST3Processor::ClassicOverdriveVST3Processor ()
{
    setControllerClass (kClassicOverdriveVST3ControllerUID);
...
tresult PLUGIN_API ClassicOverdriveVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API ClassicOverdriveVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API ClassicOverdriveVST3Processor::setActive (TBool state)
{
    if (state)
    {
...
tresult PLUGIN_API ClassicOverdriveVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API ClassicOverdriveVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API ClassicOverdriveVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API ClassicOverdriveVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
tresult PLUGIN_API ClassicOverdriveVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
const ClassicOverdriveVST3Processor::DescriptorArray& ClassicOverdriveVST3Processor::descriptors () noexcept
{
    return DSP::getParameterDescriptors ();
}

void ClassicOverdriveVST3Processor::applyAllParametersToDSP () noexcept
{
    for (const auto& descriptor : descriptors ())
    {
        applyParameterToDSP (
...
void ClassicOverdriveVST3Processor::applyParameterToDSP (
    Vst::ParamID id,
    ...
    for (auto& processor : processors_)
    {
        switch (id)
        {
            ...
            case PedalParameterIDs::PreHighPassFrequency:
                ...
                break;
            case PedalParameterIDs::PreMidGain:
                processor.setMidHumpGainDb (realValue);
                break;
            case PedalParameterIDs::PreMidFrequency:
                processor.setMidHumpFrequencyHz (realValue);
                break;
            case PedalParameterIDs::Knee:
                processor.setSoftness (static_cast<float> (normalizedValue));
                break;
...
float ClassicOverdriveVST3Processor::normalizedToReal (
    Vst::ParamID id,
    Vst::ParamValue normalizedValue) noexcept
{
...
int ClassicOverdriveVST3Processor::normalizedToIndex (
    Vst::ParamID id,
...
void ClassicOverdriveVST3Processor::clearRemainingOutputs (Vst::ProcessData& data, int32 firstBus) noexcept
{
