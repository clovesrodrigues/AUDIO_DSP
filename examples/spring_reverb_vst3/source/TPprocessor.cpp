...
SpringReverbVST3Processor::SpringReverbVST3Processor ()
{
    setControllerClass (kSpringReverbVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API SpringReverbVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API SpringReverbVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API SpringReverbVST3Processor::setActive (TBool state)
{
    if (state)
        resetDSP ();
...
tresult PLUGIN_API SpringReverbVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API SpringReverbVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API SpringReverbVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API SpringReverbVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
tresult PLUGIN_API SpringReverbVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void SpringReverbVST3Processor::registerParameters () noexcept
{
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        ...
        ParameterRange<float> {0.0f, 1.0f, 0.25f}, nullptr, 0, "mix", "%", "Spring Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamDwell, "Dwell", "Tank Dwell", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.50f}, nullptr, 0, "dwell", "%", "Spring Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamTone, "Tone", "Tank Tone", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.60f}, nullptr, 0, "tone", "%", "Spring Reverb", 2));
}

void SpringReverbVST3Processor::prepareDSP () noexcept
{
    dsp_.prepare (dspContext_);
}

void SpringReverbVST3Processor::resetDSP () noexcept
{
    dsp_.reset ();
}

void SpringReverbVST3Processor::applyParametersToDSP () noexcept
{
    ...
    dsp_.setMix (mix_);
    dsp_.setTone (tone_);
    dsp_.setTension (dwell_);
    dsp_.setSpringLength (std::max (0.35f, dwell_));
}
