...
Exemplo VST3 usando `CV_DSP/Spectral/STFT.hpp` e a classe `cvdsp::spectral::STFT<float, N>`.
...
```bash
cmake -S examples/stft_vst3 -B build/stft_vst3 -G "MinGW Makefiles"
cmake --build build/stft_vst3 --parallel
```

Artefato Windows esperado:

```text
build/stft_vst3/out/stft_vst3/stft_vst3.vst3
```
