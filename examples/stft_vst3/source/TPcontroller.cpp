...
tresult PLUGIN_API STFTVST3Controller::initialize (FUnknown* context)
{
    tresult result = EditControllerEx1::initialize (context);
    if (result != kResultOk)
        return result;

    auto* window = new Vst::StringListParameter (STR16 ("Windowing"), kParamSTFTVST3Window);
    ...
    auto* fftSize = new Vst::StringListParameter (STR16 ("FFT Size"), kParamSTFTVST3FFTSize);
...
tresult PLUGIN_API STFTVST3Controller::terminate ()
{
    return EditControllerEx1::terminate ();
}

tresult PLUGIN_API STFTVST3Controller::setComponentState (IBStream* state)
{
    if (!state)
        return kResultFalse;
    ...
    if (!streamer.readFloat (window) || !streamer.readFloat (fftSize))
        return kResultFalse;
    setParamNormalized (kParamSTFTVST3Window, normalizeEnum (window, 5));
    setParamNormalized (kParamSTFTVST3FFTSize, normalizeEnum (fftSize, 2));
    return kResultOk;
}

tresult PLUGIN_API STFTVST3Controller::setState (IBStream* /*state*/)
{
    return kResultTrue;
}

tresult PLUGIN_API STFTVST3Controller::getState (IBStream* /*state*/)
{
    return kResultTrue;
}

IPlugView* PLUGIN_API STFTVST3Controller::createView (FIDString name)
{
