...
TapeSatVST3Processor::TapeSatVST3Processor ()
{
    setControllerClass (kTapeSatVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API TapeSatVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API TapeSatVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API TapeSatVST3Processor::setActive (TBool state)
{
    if (state)
    {
...
tresult PLUGIN_API TapeSatVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API TapeSatVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API TapeSatVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API TapeSatVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
    ...
    (void)parameters_.setImmediateReal (kParamTapeSatDrive, drive);
    (void)parameters_.setImmediateReal (kParamTapeSatBias, bias);
    (void)parameters_.setImmediateReal (kParamTapeSatMix, mix);
    applyParametersToDSP ();
    return kResultOk;
}

tresult PLUGIN_API TapeSatVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void TapeSatVST3Processor::registerParameters () noexcept
{
    using namespace cvdsp::manager;
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamTapeSatDrive, "Drive", "Drive",
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamTapeSatBias, "Bias", "Bias",
        ParameterUnit::None, ParameterScale::Linear, kParamFlags,
        {-1.0f, 1.0f, 0.0f, 0.0f, 1.0f}, nullptr, 0, "bias", "", "Saturation", 2),
        ParameterSmoothingMode::Linear);
    (void)parameters_.registerParameter (ParameterDescriptor<float> (kParamTapeSatMix, "Mix", "Mix",
        ParameterUnit::Percent, ParameterScale::Linear, kParamFlags,
        {0.0f, 1.0f, 1.0f, 0.0f, 1.0f}, nullptr, 0, "mix", "", "Saturation", 2),
        ParameterSmoothingMode::Linear);
}

void TapeSatVST3Processor::applyParametersToDSP () noexcept
{
    drive_ = parameters_.getCurrentReal (kParamTapeSatDrive);
    bias_ = parameters_.getCurrentReal (kParamTapeSatBias);
    outputScale_ = 1.0f / (1.0f + (0.04f * (drive_ - 1.0f)));
    mix_ = parameters_.getCurrentReal (kParamTapeSatMix);
...
float TapeSatVST3Processor::processSample (float input, std::size_t channel) noexcept
{
