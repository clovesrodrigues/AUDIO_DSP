...
TwinReverbVST3Processor::TwinReverbVST3Processor ()
{
    setControllerClass (kTwinReverbVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API TwinReverbVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API TwinReverbVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API TwinReverbVST3Processor::setActive (TBool state)
{
    if (state)
        resetDSP ();
...
tresult PLUGIN_API TwinReverbVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API TwinReverbVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API TwinReverbVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API TwinReverbVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
tresult PLUGIN_API TwinReverbVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void TwinReverbVST3Processor::registerParameters () noexcept
{
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        ...
        ParameterRange<float> {0.0f, 1.0f, 0.25f}, nullptr, 0, "mix", "%", "Twin Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamDwell, "Dwell", "Tank Dwell", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.50f}, nullptr, 0, "dwell", "%", "Twin Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamTone, "Tone", "Tank Tone", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.60f}, nullptr, 0, "tone", "%", "Twin Reverb", 2));
}

void TwinReverbVST3Processor::prepareDSP () noexcept
{
    dsp_.prepare (dspContext_);
}

void TwinReverbVST3Processor::resetDSP () noexcept
{
    dsp_.reset ();
}

void TwinReverbVST3Processor::applyParametersToDSP () noexcept
{
