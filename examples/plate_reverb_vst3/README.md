...
`plate_reverb_vst3` is an individual CV_DSP VST3 integration for `cvdsp::reverb::PlateReverb<float>`.
...
`Damping` darkens the plate feedback path and inversely drives the plate brightness control.
...
Host automation
  ...
  -> cvdsp::reverb::PlateReverb<float>
```
...
```bat
cmake -S examples/plate_reverb_vst3 -B build/plate_reverb_vst3 -G "MinGW Makefiles" -DPLATE_REVERB_VST3_ENABLE_CV_GUI=ON
cmake --build build/plate_reverb_vst3 --config Release
```

Or invoke `mingw32-make` directly after configuration:

```bat
cd build\plate_reverb_vst3
mingw32-make -j4
```
...
2. Insert `plate_reverb_vst3` on a stereo audio track.
