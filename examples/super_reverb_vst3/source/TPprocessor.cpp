...
SuperReverbVST3Processor::SuperReverbVST3Processor ()
{
    setControllerClass (kSuperReverbVST3ControllerUID);
    registerParameters ();
...
tresult PLUGIN_API SuperReverbVST3Processor::initialize (FUnknown* context)
{
...
tresult PLUGIN_API SuperReverbVST3Processor::terminate ()
{
    return AudioEffect::terminate ();
}

tresult PLUGIN_API SuperReverbVST3Processor::setActive (TBool state)
{
    if (state)
        resetDSP ();
...
tresult PLUGIN_API SuperReverbVST3Processor::setupProcessing (Vst::ProcessSetup& newSetup)
{
...
tresult PLUGIN_API SuperReverbVST3Processor::canProcessSampleSize (int32 symbolicSampleSize)
{
    return symbolicSampleSize == Vst::kSample32 ? kResultTrue : kResultFalse;
}

tresult PLUGIN_API SuperReverbVST3Processor::process (Vst::ProcessData& data)
{
...
tresult PLUGIN_API SuperReverbVST3Processor::setState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
tresult PLUGIN_API SuperReverbVST3Processor::getState (IBStream* state)
{
    if (!state)
        return kResultFalse;
...
void SuperReverbVST3Processor::registerParameters () noexcept
{
    ...
    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        ...
        ParameterRange<float> {0.0f, 1.0f, 0.25f}, nullptr, 0, "mix", "%", "Super Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamDwell, "Dwell", "Tank Dwell", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.50f}, nullptr, 0, "dwell", "%", "Super Reverb", 2));

    (void)parameters_.registerParameter (ParameterDescriptor<float> (
        kParamTone, "Tone", "Tank Tone", ParameterUnit::Percent, ParameterScale::Percentage, kParamFlags,
        ParameterRange<float> {0.0f, 1.0f, 0.60f}, nullptr, 0, "tone", "%", "Super Reverb", 2));
}

void SuperReverbVST3Processor::prepareDSP () noexcept
{
    dsp_.prepare (dspContext_);
}

void SuperReverbVST3Processor::resetDSP () noexcept
{
    dsp_.reset ();
}

void SuperReverbVST3Processor::applyParametersToDSP () noexcept
{
    ...
    dsp_.setTone (tone_);
    dsp_.setWidth (0.35f + (dwell_ * 0.65f));
